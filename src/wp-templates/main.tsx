...
/**
 * Main/Index Template - Used for homepage/front page
 * This template handles the root URL (/)
 */
const Main: FaustTemplate<GetPageQuery> = (props) => {
	// LOADING ----------
	if (props.loading) {
		return (
	...
	// If no page data, show a simple welcome message
	if (!props.data?.page) {
		return (
			<PageLayout
				...
				pageTitle={
					// @ts-ignore
					props.data?.generalSettings?.title || 'Bosh sahifa'
				}
				generalSettings={
			...
			>
				<div className="container mx-auto px-4 py-12">
					<main className="prose mx-auto lg:prose-lg dark:prose-invert">
						<h1 className="text-4xl font-bold text-gray-900 dark:text-white mb-4">
							{/* @ts-ignore */}
							{props.data?.generalSettings?.title || 'Xush kelibsiz'}
						</h1>
						<p className="text-lg text-gray-600 dark:text-gray-400">
							{/* @ts-ignore */}
							{props.data?.generalSettings?.description || 'Bosh sahifa'}
						</p>
					</main>
				</div>
			</PageLayout>
